 ...
 * 内容覆盖六级标题、段落、有序与无序列表、独立图片以及全部表格预设，
 * 使模板设置的每一项都能在预览中看到真实效果。
 */
...
<!-- yibiao:block -->
<h2 id="tpl_h_003">进度与成果控制</h2>

<!-- yibiao:block -->
<p id="tpl_p_003">我方将合同工期分解为可检查的阶段节点，通过周计划跟踪、偏差预警和资源动态调整控制实施节奏。计划执行情况纳入项目例会，影响关键节点的问题由项目经理牵头协调解决。</p>

<!-- yibiao:block -->
<table id="tpl_tbl_001" data-yb-preset="headerRow">
...
<!-- yibiao:block -->
<h3 id="tpl_h_004">阶段成果要求</h3>

<!-- yibiao:block -->
<p id="tpl_p_004">每项阶段成果均明确责任人、完成时限和检查要求。项目资料与现场工作同步形成，确保过程记录真实完整、成果状态清晰可追溯。</p>

<!-- yibiao:block -->
<ul id="tpl_ul_001"><li>建立项目启动、过程检查和验收交付的闭环机制。</li><li>按周同步风险、进度和资源需求，确保实施节奏可控。</li><li>保留关键过程记录，便于后续审查和复盘。</li></ul>

<!-- yibiao:block -->
<h4 id="tpl_h_005">资料同步要求</h4>

<!-- yibiao:block -->
<p id="tpl_p_005">整理招标文件、现状资料和接口清单，随现场工作同步形成过程记录，支撑后续方案细化与成果复核。</p>

<!-- yibiao:block -->
<h5 id="tpl_h_006">记录归档要求</h5>

<!-- yibiao:block -->
<p id="tpl_p_006">按项目阶段整理归档目录、会议纪要、问题闭环记录和验收支撑材料，保证版本统一、状态可查。</p>

<!-- yibiao:block -->
<h6 id="tpl_h_007">过程复核要点</h6>

<!-- yibiao:block -->
<p id="tpl_p_007">对关键节点的确认材料、实施记录和交付清单进行复核，确保过程资料完整一致、责任可追溯。</p>

<!-- yibiao:block -->
<h2 id="tpl_h_008">质量与风险保障</h2>

<!-- yibiao:block -->
<p id="tpl_p_008">质量管理贯穿项目实施全过程。我方以质量策划为先导，以样板确认和工序检查为抓手，以成果复核和问题闭环为保障，执行编制、复核、批准三级审查制度；对关键工序设置质量控制点，未经检查确认不得转入下一阶段。</p>
...
<figure id="tpl_fig_001" data-yb-generation="htmlImage" data-yb-size="wide">
  ...
  <img src="${standardQualityControlUrl}" alt="项目全过程质量管控架构图">
  <figcaption>项目全过程质量管控架构</figcaption>
</figure>

<!-- yibiao:block -->
<p id="tpl_p_009">针对进度、质量和协同风险，项目组分别建立节点预警、成果复核和重大事项升级机制。风险发生后立即评估影响，采取资源调整、技术复核或专项协调措施，并持续跟踪直至关闭。</p>

<!-- yibiao:block -->
<table id="tpl_tbl_002" data-yb-preset="headerColumn">
...
<!-- yibiao:block -->
<h1 id="tpl_h_009">图表与图组展示</h1>

<!-- yibiao:block -->
<p id="tpl_p_010">本章集中展示图文混排、多图并列和图表组合的排版效果，用于核对图片宽度、对齐方式、图注样式以及各类表格预设在当前模板下的实际表现。</p>

<!-- yibiao:block -->
<table id="tpl_tbl_003" data-yb-preset="imageText">
  <caption>项目实施总控模型</caption>
  <tbody>
    <tr>
      <td><figure id="tpl_fig_002" data-yb-generation="aiImage" data-yb-size="tall"><template data-yb-role="prompt">生成 3:4 竖向大型工程项目实施总控数字孪生场景图，以轴测视角展示施工区域、临建设施、物流通道、作业分区和管理节点；主体集中在画面中央安全区域，蓝灰工程视觉，无文字、标志和水印。</template><img src="${visualMasterPlanSceneUrl}" alt="项目实施总控数字孪生场景图"><figcaption>项目实施总控场景</figcaption></figure></td>
...
<!-- yibiao:block -->
<table id="tpl_tbl_004" data-yb-preset="headerRowAndColumn">
...
<!-- yibiao:block -->
<table id="tpl_tbl_005" data-yb-preset="threeImages">
  <caption>质量、工作分解与技术架构</caption>
  <tbody>
    <tr>
      <td><figure id="tpl_fig_003" data-yb-generation="htmlImage" data-yb-size="square"><template data-yb-role="prompt">生成 1:1 方形全过程质量控制闭环信息图，展示输入审查、样板确认、工序检查、问题整改、复核销项和持续改进的循环关系；采用专业蓝白信息图风格，不使用文字，仅用图标、节点和箭头表达，无水印。</template><img src="${visualQualityClosedLoopUrl}" alt="全过程质量控制闭环信息图"><figcaption>全过程质量控制闭环</figcaption></figure></td>
      <td><figure id="tpl_fig_004" data-yb-generation="mermaid" data-yb-size="square"><template data-yb-role="prompt">生成 1:1 方形项目工作分解结构思维导图，以项目目标为中心，向外展开项目管理、深化设计、采购供应、现场实施、调试验收和资料移交六个一级分支；不使用文字，仅用层级色块、图标和连线表达，无水印。</template><img src="${visualWbsMindmapUrl}" alt="项目工作分解结构思维导图"><figcaption>WBS 工作分解结构</figcaption></figure></td>
      <td><figure id="tpl_fig_005" data-yb-generation="htmlImage" data-yb-size="square"><template data-yb-role="prompt">生成 1:1 方形项目技术架构图，分层展示基础设施层、设备接入层、数据传输层、平台服务层和业务应用层，体现上下行数据流和安全边界；采用专业蓝图风格，不使用文字，仅用模块、图标和连线表达，无水印。</template><img src="${visualTechnicalArchitectureUrl}" alt="项目分层技术架构图"><figcaption>分层技术架构</figcaption></figure></td>
    </tr>
  </tbody>
</table>

<!-- yibiao:block -->
<table id="tpl_tbl_006" data-yb-preset="fourImages">
  <caption>组织、资源、实施与交付图组</caption>
  <tbody>
    <tr>
      <td><figure id="tpl_fig_006" data-yb-generation="mermaid" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 横向工程项目组织架构图，以项目经理部为核心，下设技术管理、工程实施、质量安全、物资设备和资料交付五个专业组，并体现汇报与协同关系；不使用文字，仅用人物图标、层级节点和连接线表达，无水印。</template><img src="${visualProjectOrganizationUrl}" alt="工程项目组织架构图"><figcaption>项目组织架构</figcaption></figure></td>
      <td><figure id="tpl_fig_007" data-yb-generation="aiImage" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 横向工程项目设备与工器具配置专业轴测图，分区展示运输吊装、安装加工、检测调试和安全防护四类资源，器材摆放有序；蓝灰工程视觉，无文字、品牌和水印。</template><img src="${visualEquipmentConfigurationUrl}" alt="工程设备与工器具配置轴测图"><figcaption>设备资源配置</figcaption></figure></td>
    </tr>
    <tr>
      <td><figure id="tpl_fig_008" data-yb-generation="aiImage" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 横向复杂工程现场多专业协同实施剖切场景图，展示结构、机电、管线和设备安装在不同作业面有序穿插，具备真实工程细节；专业工程可视化风格，无文字、品牌和水印。</template><img src="${visualSiteImplementationUrl}" alt="多专业协同现场实施剖切场景图"><figcaption>多专业协同实施</figcaption></figure></td>
      <td><figure id="tpl_fig_009" data-yb-generation="htmlImage" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 横向项目验收与移交流程图，展示专业自检、联合预验收、问题整改、正式验收、资料归档和运维移交的顺序及反馈闭环；采用专业蓝白流程图风格，不使用文字，仅用图标、节点和箭头表达，无水印。</template><img src="${visualAcceptanceWorkflowUrl}" alt="项目验收与移交流程图"><figcaption>验收与移交闭环</figcaption></figure></td>
    </tr>
  </tbody>
</table>

<!-- yibiao:block -->
<table id="tpl_tbl_007" data-yb-preset="plain">
  ...
  <tbody>
    ...
    <tr><td colspan="2"><figure id="tpl_fig_010" data-yb-generation="htmlImage" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 全景项目总体进度甘特图，体现准备、深化、采购、实施、调试和验收六类工作在时间轴上的穿插关系、关键里程碑和关键路径；采用专业项目管理图表风格，不使用文字和数字，仅用分组色带、进度条和菱形节点表达，无水印。</template><img src="${visualScheduleGanttUrl}" alt="项目总体进度甘特图"><figcaption>总体进度甘特计划</figcaption></figure></td></tr>
  </tbody>
</table>

<!-- yibiao:block -->
<p id="tpl_p_011">我方将以明确的组织体系、可执行的进度计划和严格的质量控制措施保障项目顺利实施，并接受采购人对全过程工作的监督与考核。</p>`;
