...
/** 排版风格模板注册表；displayHtml 与 coreHtml 是同一风格的成对模板。 */
...
<table id="visual_tbl_002" data-yb-preset="imageText">
  ...
  <tbody>
    <tr>
      <td><figure id="visual_fig_001" data-yb-generation="aiImage" data-yb-size="tall"><template data-yb-role="prompt">生成 3:4 竖向大型工程项目实施总控数字孪生场景图，以轴测视角展示施工区域、临建设施、物流通道、作业分区和管理节点；主体集中在画面中央安全区域，蓝灰工程视觉，无文字、标志和水印，适合 A3 横版双列页面的单列栏宽。</template><img src="${visualMasterPlanSceneUrl}" alt="项目实施总控数字孪生场景图"><figcaption>项目实施总控场景</figcaption></figure></td>
...
<table id="visual_tbl_003" data-yb-preset="plain">
  ...
  <tbody>
    ...
    <tr><td colspan="2"><figure id="visual_fig_002" data-yb-generation="htmlImage" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 全景项目总体进度甘特图，体现准备、深化、采购、实施、调试和验收六类工作在时间轴上的穿插关系、关键里程碑和关键路径；采用专业项目管理图表风格，不使用文字和数字，仅用分组色带、进度条和菱形节点表达，关键内容置于画面安全区域，无水印。</template><img src="${visualScheduleGanttUrl}" alt="项目总体进度甘特图"><figcaption>总体进度甘特计划</figcaption></figure></td></tr>
  </tbody>
</table>
...
<table id="visual_tbl_004" data-yb-preset="threeImages">
  ...
  <tbody>
    <tr>
      <td><figure id="visual_fig_003" data-yb-generation="htmlImage" data-yb-size="square"><template data-yb-role="prompt">生成 1:1 方形全过程质量控制闭环信息图，展示输入审查、样板确认、工序检查、问题整改、复核销项和持续改进的循环关系；采用专业蓝白信息图风格，不使用文字，仅用图标、节点和箭头表达，关键内容置于画面安全区域，无水印。</template><img src="${visualQualityClosedLoopUrl}" alt="全过程质量控制闭环信息图"><figcaption>全过程质量控制闭环</figcaption></figure></td>
      <td><figure id="visual_fig_004" data-yb-generation="mermaid" data-yb-size="square"><template data-yb-role="prompt">生成 1:1 方形项目工作分解结构思维导图，以项目目标为中心，向外展开项目管理、深化设计、采购供应、现场实施、调试验收和资料移交六个一级分支，并继续分解关键工作；不使用文字，仅用层级色块、图标和连线表达，关键内容置于画面安全区域，无水印。</template><img src="${visualWbsMindmapUrl}" alt="项目工作分解结构思维导图"><figcaption>WBS 工作分解结构</figcaption></figure></td>
      <td><figure id="visual_fig_005" data-yb-generation="htmlImage" data-yb-size="square"><template data-yb-role="prompt">生成 1:1 方形项目技术架构图，分层展示基础设施层、设备接入层、数据传输层、平台服务层和业务应用层，体现上下行数据流和安全边界；采用专业蓝图风格，不使用文字，仅用模块、图标和连线表达，关键内容置于画面安全区域，无水印。</template><img src="${visualTechnicalArchitectureUrl}" alt="项目分层技术架构图"><figcaption>分层技术架构</figcaption></figure></td>
    </tr>
  </tbody>
</table>
...
<table id="visual_tbl_005" data-yb-preset="fourImages">
  ...
  <tbody>
    <tr>
      <td><figure id="visual_fig_006" data-yb-generation="mermaid" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 横向工程项目组织架构图，以项目经理部为核心，下设技术管理、工程实施、质量安全、物资设备和资料交付五个专业组，并体现汇报与协同关系；不使用文字，仅用人物图标、层级节点和连接线表达，关键内容置于画面安全区域，无水印。</template><img src="${visualProjectOrganizationUrl}" alt="工程项目组织架构图"><figcaption>项目组织架构</figcaption></figure></td>
      <td><figure id="visual_fig_007" data-yb-generation="aiImage" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 横向工程项目设备与工器具配置专业轴测图，分区展示运输吊装、安装加工、检测调试和安全防护四类资源，器材摆放有序；关键内容置于画面安全区域，蓝灰工程视觉，无文字、品牌和水印。</template><img src="${visualEquipmentConfigurationUrl}" alt="工程设备与工器具配置轴测图"><figcaption>设备资源配置</figcaption></figure></td>
    </tr>
    <tr>
      <td><figure id="visual_fig_008" data-yb-generation="aiImage" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 横向复杂工程现场多专业协同实施剖切场景图，展示结构、机电、管线和设备安装在不同作业面有序穿插，具备真实工程细节；关键内容置于画面安全区域，专业工程可视化风格，无文字、品牌和水印。</template><img src="${visualSiteImplementationUrl}" alt="多专业协同现场实施剖切场景图"><figcaption>多专业协同实施</figcaption></figure></td>
      <td><figure id="visual_fig_009" data-yb-generation="htmlImage" data-yb-size="panorama"><template data-yb-role="prompt">生成 16:9 横向项目验收与移交流程图，展示专业自检、联合预验收、问题整改、正式验收、资料归档和运维移交的顺序及反馈闭环；采用专业蓝白流程图风格，不使用文字，仅用图标、节点和箭头表达，关键内容置于画面安全区域，无水印。</template><img src="${visualAcceptanceWorkflowUrl}" alt="项目验收与移交流程图"><figcaption>验收与移交闭环</figcaption></figure></td>
    </tr>
  </tbody>
</table>
...
<table id="visual_tbl_008" data-yb-preset="imageText">
  ...
  <tbody>
    <tr>
      <td><figure id="visual_fig_010" data-yb-generation="htmlImage" data-yb-size="wide"><template data-yb-role="prompt">生成 3:2 横向工程项目质量安全联合检查流程信息图，依次体现策划检查、样板确认、工序巡检、问题登记、整改复核和闭环归档；采用专业蓝白工程图标风格，不使用文字，仅用图标、节点和箭头表达，关键内容置于画面安全区域，无水印。</template><img src="${standardQualityControlUrl}" alt="质量安全联合检查流程信息图"><figcaption>质量安全联合检查闭环</figcaption></figure></td>
