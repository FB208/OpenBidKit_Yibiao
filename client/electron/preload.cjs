...
const bridge = {
  ...
  knowledgeBase: {
    ...
    createFolder: (name, type, parentId) => ipcRenderer.invoke('knowledge-base:create-folder', name, type, parentId),
    renameFolder: (folderId, name) => ipcRenderer.invoke('knowledge-base:rename-folder', folderId, name),
    reorderFolder: (draggedFolderId, targetFolderId, position, parentId) => ipcRenderer.invoke('knowledge-base:reorder-folder', draggedFolderId, targetFolderId, position, parentId),
    deleteFolder: (folderId) => ipcRenderer.invoke('knowledge-base:delete-folder', folderId),
    moveFolder: (folderId, targetParentId) => ipcRenderer.invoke('knowledge-base:move-folder', folderId, targetParentId),
    ...
    deleteItem: (documentId, itemId) => ipcRenderer.invoke('knowledge-base:delete-item', documentId, itemId),
    listSnippets: (folderId) => ipcRenderer.invoke('knowledge-base:list-snippets', folderId),
    createSnippet: (folderId, payload) => ipcRenderer.invoke('knowledge-base:create-snippet', folderId, payload),
    updateSnippet: (snippetId, partial) => ipcRenderer.invoke('knowledge-base:update-snippet', snippetId, partial),
    deleteSnippet: (snippetId) => ipcRenderer.invoke('knowledge-base:delete-snippet', snippetId),
    images: {
      list: (folderId) => ipcRenderer.invoke('knowledge-base:list-images', folderId),
      create: (folderId, payload) => ipcRenderer.invoke('knowledge-base:create-image', folderId, payload),
      update: (imageId, partial) => ipcRenderer.invoke('knowledge-base:update-image', imageId, partial),
      delete: (imageId) => ipcRenderer.invoke('knowledge-base:delete-image', imageId),
      getFile: (imageId) => ipcRenderer.invoke('knowledge-base:get-image-file', imageId),
      createFromClipboard: (folderId, payload) => ipcRenderer.invoke('knowledge-base:create-image-from-clipboard', folderId, payload),
    },
    onEvent: (callback) => {
      const listener = (_event, payload) => callback(payload);
  ...
  technicalPlan: {
    ...
    saveTenderStarredSections: (starredSections) => ipcRenderer.invoke('technical-plan:save-tender-starred-sections', starredSections),
  ...
  businessBid: {
    ...
    readTenderMarkdown: () => ipcRenderer.invoke('business-bid:read-tender-markdown'),
    associateTechnicalPlan: () => ipcRenderer.invoke('business-bid:associate-technical-plan'),
    disassociateTechnicalPlan: () => ipcRenderer.invoke('business-bid:disassociate-technical-plan'),
    hasTechnicalPlan: () => ipcRenderer.invoke('business-bid:has-technical-plan'),
    updateStep: (step) => ipcRenderer.invoke('business-bid:update-step', step),
    saveOutlineConfig: (payload) => ipcRenderer.invoke('business-bid:save-outline-config', payload),
    saveOutline: (outlineData) => ipcRenderer.invoke('business-bid:save-outline', outlineData),
    saveGlobalFacts: (globalFacts) => ipcRenderer.invoke('business-bid:save-global-facts', globalFacts),
    saveClauseItems: (clauseItems) => ipcRenderer.invoke('business-bid:save-clause-items', clauseItems),
    saveContentGenerationOptions: (options) => ipcRenderer.invoke('business-bid:save-content-generation-options', options),
  ...
  tasks: {
    ...
    startBusinessClauseAnalysis: (payload) => ipcRenderer.invoke('tasks:start-business-clause-analysis', payload),
    startBusinessClauseRegeneration: (payload) => ipcRenderer.invoke('tasks:start-business-clause-regeneration', payload),
    startBusinessOutlineGeneration: (payload) => ipcRenderer.invoke('tasks:start-business-outline-generation', payload),
    startBusinessGlobalFactsGeneration: (payload) => ipcRenderer.invoke('tasks:start-business-global-facts-generation', payload),
    startBusinessContentGeneration: (payload) => ipcRenderer.invoke('tasks:start-business-content-generation', payload),
    pauseBusinessContentGeneration: () => ipcRenderer.invoke('tasks:pause-business-content-generation'),
