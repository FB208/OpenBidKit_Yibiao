...
function globalFactsInstructions(mode) {
  const requirement = mode === 'placeholder'
    ? '放着我来模式：保留必要事项，不确定事实使用【待填写】，已有占位符原样沿用。'
    : mode === 'omit'
      ? '别招欠模式：保留必要事项，不确定事实使用笼统表达，不把笼统口径补成未经提供的具体事实，不编造具体人员、时间、地点、业绩、证书或规格型号。'
      : '胡咧咧模式：参考材料缺少具体事实时，允许结合项目补充编造所需信息；新增人员、时间、参数等口径须在全文保持一致。';
  return `${requirement} 全局事实设定优先，不能用补充内容覆盖已有设定或与其矛盾；需要具体事实时先检索核对，不能仅因没有阅读就认定事实缺失。`;
}

// 正文写作共用规则，事实要求由编排决策中的同一段中文说明提供。
function writingInstructions(hasKnowledgeBase) {
  return `你是专业的投标文件正文编写专家。内容专业、准确、具体、可执行，围绕章节描述和写作重点展开；语言正式朴实，不写宣传口号、空话和重复凑字内容。\n参考资料改写为当前项目语境，不提及${hasKnowledgeBase ? '知识库、' : ''}历史文档或素材来源；事实设定优先，不得与其矛盾。\n只输出受限 HTML 正文，不输出 Markdown、代码围栏、外层章节标题或解释。内部层次用普通段落、列表或无编号加粗引导语；有序列表仅用于步骤、流程和时间顺序。`;
}
...
2. ${hasKnowledgeBase ? '知识库/包含用户选中的全部文档，索引提供编排知识条目与文件的对应关系；知识库和' : ''}全局事实设定.md是参考项，用到时再搜索和读取相关内容，不要求全文通读。具体事实以全局事实设定为准，并遵守正文编排决策.json中的 global_facts_requirements（当前事实模式的中文要求）；不能仅因没有阅读就认定事实缺失。
