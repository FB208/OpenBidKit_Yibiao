...
import type { TechnicalPlanState, TechnicalPlanTenderFile, TechnicalPlanTenderSourceFile } from '../types';
...
function DocumentFilePill({ file, onRemove, removeDisabled = false }: { file: TechnicalPlanTenderFile | TechnicalPlanTenderSourceFile; onRemove?: () => void; removeDisabled?: boolean }) {
  return (
    <UploadFilePill
      badge="MD"
      name={file.fileName}
...
function DocumentAnalysisPage({
  ...
  }, [activeDocumentTab, firstTenderSourceId, tenderFiles]);
  ...
  const activeTenderSource = activeDocumentTab.startsWith('tender:')
    ...
    : null;
  const activeFile = activeTenderSource || tenderFile;
  const activeMarkdown = activeTenderSource
    ? tenderSourceMarkdowns[activeTenderSource.id] || ''
    : tenderMarkdown;
  const readerEmptyText = '当前步骤只负责把招标文件解析成 Markdown，后续步骤会基于这里的内容完成设置和 AI 标书理解。';
  const documentTabs = [
    ...(tenderFiles.length ? tenderFiles.map((file, index) => ({ id: `tender:${file.id}`, label: `招标文件${index + 1}` })) : [{ id: 'tender', label: '招标文件' }]),
  ];
  const hasDocumentTabs = tenderFiles.length > 1;
