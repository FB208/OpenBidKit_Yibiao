...
先读取该文件，再按用户要求使用原生 edit 修改，只修改这个小节。遵守原有受限 HTML 规范和全局事实设定；未要求调整的内容和图片保持不变，原方案表格和图片继续保留。本次新增或改写的正文禁止使用 LaTeX 语法，包括 $...$、$$...$$、\\(...\\)、\\[...\\] 及 \\frac、\\text、\\circ 等命令。公式、参数和单位使用普通文字、Unicode 数学符号及受限 HTML 的 <sup>、<sub> 表达，例如 22 ℃ ± 2 ℃、40%～65%、≥30 m<sup>3</sup>/(h·人)。参考材料中的 LaTeX 在写入正文时也须转换为上述表达，保持数值、单位和含义不变。需要配图时先保存 figure 布局，再调用 list-section-images 读取本小节最新图片清单；image_id 沿用清单标识，不自行重编，原图和未要求调整的有效图片直接复用。需要修正结构或提示词时可 read/edit 后重新提取，无须自行编写扫描脚本。使用 generate-section-images 的 images 一次提交本次全部待生成 AI、HTML、Mermaid 图片，不按类型或固定小批次拆分。每项提供清单 image_id、kind（ai/html/mermaid）和准确的内容与数据 prompt；AI 另填与画框比例一致的具体 size，HTML 另填与正文画框一致的 frame_size。程序向现有生图及文本队列同时提交，源码逐张完成后立即本地转图，超限自动排队。按 results 中的 image_id 对应正文图片，逐项检查 status、stage 和 error，按工具说明处理未成功项；success 图片直接回填。有 source_file 的失败或未完成项直接读取、必要时修改源码后批量调用对应 render 工具，每项填 image_id、source_file，HTML 另填 frame_size；单张也用 images 数组。无源码的失败项才重试生成。问题解决后调用 apply-section-images 批量回填，每项提供清单 image_id、图片工具返回的 asset_ref 和清单原引用 previous_asset_ref（未填写时为空字符串）；程序只更新对应 img 的引用。状态非 success 的项不提交，引用变化时刷新清单后处理。仅重试失败或需要修正的项，不覆盖已有图片文件。暂停恢复时核对会话中逐项结果与最新清单，复用已完成图片及源码，只混合提交无源码的剩余生成任务，已有源码的剩余项使用对应 render 工具。
