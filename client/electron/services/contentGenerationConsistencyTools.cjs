...
完整阅读全局事实设定，冲突以全局事实设定为准，并阅读编排决策中的 global_facts_requirements（当前事实模式的中文要求）。审计阶段只修复矛盾，优先采用全局事实及已有依据统一口径；即使生成阶段允许补充编造缺失信息，也不能在审计时重新猜一个值解决冲突或覆盖已有设定。没有确定依据时记录尚未解决的问题，不将【待填写】替换为猜测值。正常场景差异不是矛盾，不做无关润色。上一轮尚未解决的问题：${JSON.stringify(state.remaining_issues || [])}。
