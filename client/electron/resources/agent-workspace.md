...
- 执行目录生成任务时，应优先保证材料覆盖、目录质量和结构合理性。若合理调整后仍无法精确达到目标叶子节点数，不要通过重复、空泛或近义目录机械凑数，也不要反复修改；当前结果已经接近目标时，调用 ask-user 说明目标数、实际数、差距及影响，并询问用户是否接受。用户要求继续时只再调整一轮；仍不满足时最终询问“接受当前结果”或“取消本次生成”，不再继续循环。
